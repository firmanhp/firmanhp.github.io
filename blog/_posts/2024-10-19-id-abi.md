...
fungsi. Ambil contoh kode C++ berikut, kode utama kita (`main.cpp`) akan membuat
...
Andaikan sang pembuat library ingin meng-update librarynya. Kita andaikan update
...
Walaupun ada member dalam scope `private` yang tidak akan terlihat oleh kode di
luar, namun tetap mempengaruhi struktur memory `MyObject` kita.
...
Apabila dijalankan dengan `my_library` versi 1:
```
My program thinks that sizeof(MyObject) = 2
My library thinks that sizeof(MyObject) = 2
```

Apabila dijalankan dengan `my_library` versi 2:
```
My program thinks that sizeof(MyObject) = 2
My library thinks that sizeof(MyObject) = 3
```

Maka sudah terlihat ada perbedaan dari pemahaman `my_library` versi 2.
...
pahami, maka yang terjadi adalah:
...
dispesifikasikan dari "high level", seperti nama fungsi yang bisa dipanggil, dan
...
seperti berikut yang harus dipenuhi kedua binary:

- Dalam memanggil fungsi ke binary lain (_system call_ juga termasuk), misalkan
  ...
  - Dimana kita harus menaruh nilai argument `a` dan `b`? Ke dalam register CPU?
    atau memory?
  - Dimana letak return value setelah memanggil fungsi tersebut?
- Dalam struktur memory sebuah object, misalkan `MyObject`:
  - Bagaimana mendefinisikan bentuk `MyObject` dalam memory yang bisa dipahami
    binary lain?
- Dan lain-lain.
...
- Menerapkan _versioning_ pada library, misalnya pada versi 7.0.0 sang developer
  ...
  - Aplikasi yang dicompile menggunakan library versi sebelum 7.0.0 tidak bisa
    dijalankan dengan library 7.0.0
